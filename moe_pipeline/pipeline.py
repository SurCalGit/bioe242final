...
class Pipeline:
    """
    K-fold cross-validation training pipeline. Training folds are further split randomly into 5 sections
    so that the experts are trained on 4 sections and the gating network is trained using predidctions
    from the (unseen) fifth section (OOF data, i.e. out-of-fold). 
    ...
    def __init__(
        ...
        loss_configs: list = None,         # default: 5 configs (entropy * 4 + load-balancing)
    ...
    def run(self, X_raw: np.ndarray, y: np.ndarray, smiles: list):
        """
        Execute the full cross validation pipeline. 
        ...
        # Outer K-fold split indices
        perm = rng.permutation(n)
        fold_val_idx  = []
        fold_pool_idx = []
        for f in range(self.n_folds):
            val_start = round(n * f / self.n_folds)
            val_end   = round(n * (f + 1) / self.n_folds)
            fold_val_idx.append(perm[val_start:val_end])
            fold_pool_idx.append(np.concatenate([perm[:val_start], perm[val_end:]]))
        ...
        for fold_idx in range(self.n_folds):
            ...
            pool_idx = fold_pool_idx[fold_idx]
            ...
            # Fit scaler on pool descriptors only
            ...
            # Inner OOF on pool -> honest predictions for gate training
            self._log("  Running inner OOF on pool...")
            oof_seed = self.seed + fold_idx * 1000
            E_oof, U_oof = self._run_inner_oof(X_pool_scaled, y_pool, smi_pool, seed=oof_seed)

            # Train full pool experts (for val prediction + uncertainties)
            self._log("  Training pool experts (for val prediction)...")
            pool_moe = MoE(seed=self.seed + fold_idx * 200 + 7)
            ...
            # Gate input for pool (train gate on pool using OOF predictions)
            X_gate_pool = np.concatenate([X_pool_raw[:, :N_DESC], E_oof, U_oof], axis=1)
            # Gate input for val (pool experts predict on val with uncertainty)
            ...
            self._log(f"  Searching {len(hyperparam_grid)} configs...")
            best_rmse = float("inf")
            best_config = None
            best_r2  = float("-inf")
            ...
            self._log(f"  Fold {fold_idx+1} best val RMSE={best_rmse:.4f} R²={best_r2:.4f}")
        ...
        rmse_vals = np.array([r["val_rmse"] for r in fold_results])
        r2_vals = np.array([r["val_r2"]   for r in fold_results])

        self._log(f"\n[CV Summary] RMSE: {rmse_vals.mean():.4f} ± {rmse_vals.std():.4f}")
        self._log(f"[CV Summary] R²:   {r2_vals.mean():.4f} ± {r2_vals.std():.4f}")

        # Select the best config (most frequently chosen, break ties by best avg RMSE)
        from collections import Counter
        config_counts = Counter(
            tuple(sorted(cfg.items())) for cfg in all_best_configs
        )
        
        # Among the most common, pick the one with lowest per-fold RMSE mean
        best_config_final = min(
            all_best_configs,
            key=lambda cfg: np.mean([
                r["val_rmse"] for r in fold_results if r["best_config"] == cfg
            ]),
        )
        ...
        self.final_moe.gate = GatingNetwork(
            ...
            lr=self.gate_lr,
            epochs=self.gate_epochs,
            weight_decay=best_config_final.get("weight_decay", 0.0),
            entropy_reg=best_config_final.get("entropy_reg", 0.0),
            load_balancing=best_config_final.get("load_balancing", False),
            oracle_reg=self.gate_oracle_reg,
        )
        ...
        return dict(
            ...
            std_r2=float(r2_vals.std()),
            fold_results=fold_results,
            best_config=best_config_final,
    ...
    def run_expert_gate_grid_search(
        ...
        oracle_reg is searched here (unlike run() where it is fixed at gate_oracle_reg).
        ...
        Same dict format as run():
            ...
            fold_results  — per-fold val metrics for the best combo
        ...
        # Gate config grid — adds oracle_reg as a searched axis
        gate_grid = [
        ...
        self._log(
            ...
            f"{n_exp} expert configs x {n_gate} gate configs "
            f"x {self.n_folds} folds = {n_exp * n_gate * self.n_folds:,} gate evals"
        )

        # fold_rmse_matrix[ecfg_idx, gcfg_idx, fold_idx]
        fold_rmse_matrix = np.full((n_exp, n_gate, self.n_folds), np.inf)
        fold_r2_matrix   = np.full((n_exp, n_gate, self.n_folds), -np.inf)
        ...
        for fold_idx in range(self.n_folds):
            ...
            # Scaler fitted once per fold on pool; shared across all expert configs
            scaler   = StandardScaler()
            desc_sc  = scaler.fit_transform(X_pool_raw[:, :N_DESC].astype(np.float64))
            X_pool_scaled = np.concatenate([desc_sc, X_pool_raw[:, N_DESC:]], axis=1)
            desc_val = scaler.transform(X_val_raw[:, :N_DESC].astype(np.float64))
            ...
            for ecfg_idx, ecfg in enumerate(expert_configs):
                self._log(f"  [Expert config {ecfg_idx+1}/{n_exp}] {ecfg}")

                # Inner OOF -> honest expert predictions for gate training
                self._log("    Running inner OOF...")
                oof_seed = self.seed + fold_idx * 1000 + ecfg_idx * 100
                E_oof, U_oof = self._run_inner_oof(
                    X_pool_scaled, y_pool, smi_pool, seed=oof_seed, expert_config=ecfg
                )

                # Pool experts -> val predictions + uncertainties
                self._log("    Training pool experts (for val prediction)...")
                pool_moe        = MoE(seed=self.seed + fold_idx * 200 + ecfg_idx * 50 + 7)
                pool_moe.scaler = scaler
                ...
                # Sweep all gate configs (MLP only — cheap)
                ...
                for gcfg_idx, gcfg in enumerate(gate_grid):
                    ...
                    fold_rmse_matrix[ecfg_idx, gcfg_idx, fold_idx] = rmse
                    fold_r2_matrix[ecfg_idx,   gcfg_idx, fold_idx] = r2

        # ── Select best (expert_config, gate_config) by mean val RMSE ──
        mean_rmse_matrix = fold_rmse_matrix.mean(axis=2)      # (n_exp, n_gate)
        best_flat        = int(np.argmin(mean_rmse_matrix))
        best_ecfg_idx, best_gcfg_idx = np.unravel_index(best_flat, mean_rmse_matrix.shape)
        ...
        rmse_vals = fold_rmse_matrix[best_ecfg_idx, best_gcfg_idx]
        r2_vals   = fold_r2_matrix[best_ecfg_idx,   best_gcfg_idx]

        self._log("\n[Grid Search Summary]")
        self._log(f"  Best mean val RMSE : {rmse_vals.mean():.4f} +/- {rmse_vals.std():.4f}")
        self._log(f"  Best mean val R2   : {r2_vals.mean():.4f}")
        ...
        fold_results = [
            dict(fold=f+1, val_rmse=float(rmse_vals[f]), val_r2=float(r2_vals[f]),
